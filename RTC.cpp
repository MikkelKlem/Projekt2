...
void setDateTime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second); // Function prototype for setting the date and time
void printDateTime();
uint8_t decToBcd(uint8_t dec); // Convert normal decimal numbers to binary coded decimal
uint8_t bcdToDec(uint8_t bcd); // Convert binary coded decimal to normal decimal numbers
...
//setDateTime(2023, 5, 21, 22, 07, 0); // Set the date and time to 10:00:00 17/5/2023 or whatever time it is currently
}
...
{
  Wire.beginTransmission(RTC_ADDRESS); //begins I2C communication
