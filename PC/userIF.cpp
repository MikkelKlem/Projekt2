...
void userIF::printMain()
{
	...
	cout << "Vælg enhed at konfigurere\n\n";
	...
	cout << "\n\n\n\n";
	cout << "5 - Gem nuværende rutine" << "          6 - Indlæs gemte rutiner \n\n";
	cout << "\n\n";
...
void userIF::printConfig(pcSettings& config)
{
	...
	cout << "Parametre der kan ændres:" << endl << endl;


	cout << "1 -	Ændre tidspunkt : Nuværende aktiveringstidspunkt:  " << config.getActivationTime() << endl << endl;

	cout << "Rutine der udføres\n";
	...
	cout << "\n\nNuværende rutine valgt: ";

	switch (config.getRutine())
	{
		case '1':
		{
...
void userIF::saveRutine(int rutinenumber)
{
	if (savedArrayNames_[rutinenumber].substr(0, savedArrayNames_[rutinenumber].find(" ")) == "Smart")
	{
		while (true)
		{
			...
			std::string tempName;
			cin >> tempName;
			if (tempName.length() <= 10 && 0 < tempName.length())
			{
				savedArrayNames_[rutinenumber] = tempName;
				for (size_t i = 0; i < 4; i++)
				{
	...
	else
	{
		cout << "\n\nDu er ved at overskrive rutine "<< rutinenumber+1 << " " << savedArrayNames_[rutinenumber] 
			 << "\nTryk 'j' for at gemme || Tryk 's' for at ændre navn || Tryk alt andet for at annulere";
		char choice = _getch();
		if (choice == 's')
		{
			while (true)
...
void userIF::displaySavedRutines()
{
	...
	system("cls");
	cout << "Indlæs gemt rutine\n\n";
	cout << "Tryk tilfældig knap for at annulere\n\n";
	cout << "\nRutine 1 - " << savedArrayNames_[0];
	cout << "\nRutine 2 - " << savedArrayNames_[1];
	cout << "\nRutine 3 - " << savedArrayNames_[2];
	cout << "\nRutine 4 - " << savedArrayNames_[3];
...
{
	system("cls");
	cout << "Du har aflsuttet programmet med følgende rutine";
	...
	for (size_t i = 0; i < 4; i++)
	{
		if (unitConfig_[i].getStatus() == true)
		{
			...
			cout << "\nRutine: " << unitConfig_[i].getRutine();
		}
	}
	setSavedName();
	//Sleep(5000);
...
{
	system("cls");
	cout << "Du har aflsuttet programmet uden at gemme, dette er den nuværedne rutine";
